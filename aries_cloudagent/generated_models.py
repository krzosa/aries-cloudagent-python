...
class PDSActivate(OpenAPISchema):
    ...
    driver = fields.String(
        required=True,
        validate=[
            OneOf(choices=["own_your_data", "local", "thcf_data_vault"], labels=[])
        ],
    )
...
class MineApplication(Application):
    service_user_data = fields.String(
        required=True, description="JSON serialized service application data"
    )
...
class PDSSettingDriver(OpenAPISchema):
    name = fields.String(
        required=True,
        validate=[
            OneOf(choices=["own_your_data", "thcf_data_vault", "local"], labels=[])
        ],
    )
...
class PDSSettingDriverOwn_your_data(OpenAPISchema):
    scope = fields.String(
        required=False, validate=[OneOf(choices=["admin", "write", "read"], labels=[])]
    )
